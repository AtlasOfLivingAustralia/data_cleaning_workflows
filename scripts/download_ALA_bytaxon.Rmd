...
Will double check on this - slacked Ely Wallis 4.Aug
Ely confirmed this is a case but ALA has added contentTypes to further delineate these categories, how exactly is not clear, so I am adding contentTypes as a field to investigate
...
Just searching each field from the `fields` vector above here https://biocache.ala.org.au/fields to make a new vector that works with the current API system. I think you can parallelise this step? Will check with Martin about this!
...
fields_v2 <-  c("id","dataResourceUid","dataResourceName",
             ...
             "contentTypes", # Adding this to check on how Material_sample is further categorised as contentTypes
             ...
             "country","stateProvince", #Assumption state = locality as oppose to conservation status
             "cl959","cl21","cl1048",
...
### Quick checks of the download

Just want to address some of the comments above, e.g. "I don't know what the difference between X and Y
...
# Not sure what is the difference between institutionID and institutionUid
invert_2022 |> select(institutionID, institutionUid) |> print(n = 50)
# Mostly empty, I think okay to remove as Uid seems to convey the same grouping levels but tidier but left as is for now

# Not sure what is difference between raw_locality, locality
invert_2022 |> select(raw_locality, locality) # It didn't download raw_locality despite it being on the field search site!

# Which fields didn't get downloaded from fields_v2
setdiff(fields_v2, colnames(invert_2022))

# "id"                   "institutionID"        "license"              "raw_scientificName"   "raw_locality"        
#  "raw_geodeticDatum"    "raw_decimalLatitude"  "raw_decimalLongitude" "raw_basisOfRecord"   

# Which vars did we download thats not in field_v2
setdiff(colnames(invert_2022),fields_v2)

# "recordID"                 "dctermsLicense"           "verbatimScientificName"   "locality1"               
# "verbatimCoordinateSystem" "verbatimLatitude"         "verbatimLongitude"        "verbatimBasisOfRecord"   
# "assertions"               "recordID1"  

# Not sure why some have suffix1?
invert_2022 |> select(ends_with("1"))
#Looks like they are mostly double ups, requesting for raw_locality will give you the same output https://biocache.ala.org.au/fields?filter=locality
all.equal(invert_2022$locality, invert_2022$locality1)
all.equal(invert_2022$recordID, invert_2022$recordID1)

# We can remove these duplicates
invert_2022 |> select(-c(locality1, recordID1)) -> invert_2022

# I think there must be some sort of alias built in, you can request a certain field but what is returned are the DwC names. raw is equivalent to verbatim for example (https://biocache.ala.org.au/fields?filter=latitude)
```
...
Some code to deal with mismatched column classes from different phylum, I suspect this might not be an issue if its in one giant tibble in the eventual download, but requires testing and but splitting phylums into lists
