...
Using other version of `remove_improper_names_v2`

```{r}
species_record_v2 <- remove_improper_names_v2(as.character(all_species),
                               allow.higher.taxa = FALSE,
                               allow.subspecies = TRUE)

is.na(species_record$updated_list) |> table() # I don't expect the function to produce the updated_list with any NAs so I don't think its neccesary to use na.omit here :) 
str(species_record$updated_list) # Data was read in as chr, I don't

# Filter records
afd_species |> dplyr::filter(VALID_NAME %in% species_record$updated_list) 
```
...
afd_species <- afd_species[which(! afd_species$VALID_NAME %in% griis_species$Supplied.Name),] #none
```

## Counts for special characters in species names

This section tallies names that contain special characters, but doesn't do anything with them
...
A stringr, readable alternative
```{r}
stringr::str_count(afd_species$VALID_NAME, pattern = regex("\\\"")) |> sum()
# stringr::str_subset(afd_species$VALID_NAME, pattern = regex("\\\"")) # Double checking we are counting the right ones
...
What I don't understand are the indices as it is just a sequence [111238:1] which should match the entirety of `afd_species$COMPLETE_NAME` but the output names are different between the 

```{r}
temp <- afd_species[which(
  duplicated(afd_species$COMPLETE_NAME) | #OR statement
...
Below is Fonti's code to figure this out
...
which(duplicated(afd_species$COMPLETE_NAME[111238:1])) 

afd_species[d1,"COMPLETE_NAME"] 
afd_species[d2,"COMPLETE_NAME"]

# I suspect the goal is flag both the first occurrence and subsequent occurrences of duplicates
# ( x <- c(1:5, 3:7, 0:8) )
# x [duplicated(x) | duplicated(x, fromLast=TRUE)] 

# Applying to species names, This will flag both first occurrence and subsequent duplicates
afd_species$COMPLETE_NAME[duplicated(afd_species$COMPLETE_NAME) | duplicated(afd_species$COMPLETE_NAME, fromLast=TRUE)]
...
Looks like some duplicates but not all of them? 5/17
...
Checks or tallies of species according to the number of words in `VALID_NAME`
