...
```{r}
species <- fread(here("data", "WoRMS_download_2022-10-01", "speciesprofile.txt"))
taxa <- fread(here("data", "WoRMS_download_2022-10-01", "taxon.txt"))
...
afd_taxonomy |> filter(! VALID_NAME %in% worms_list$scientificName) # 91,550
afd_taxonomy |> filter(VALID_NAME %in% worms_list$scientificName) # 21,046 species excluded
```

## Trying to say csv/.txt in seperate parts so its more manageable
