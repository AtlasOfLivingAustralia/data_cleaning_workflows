...
```{r}
# Quick tally of all
invert_irmng_awc_insed |> tabyl(taxonomicStatus) |> arrange(-n)

invert_irmng_awc_insed |> filter(taxonomicStatus == "accepted") -> accepted_invert_irmng

# We would be removing ~40%
(nrow(invert_irmng_awc_insed) - nrow(accepted_invert_irmng)) / nrow(invert_irmng_awc_insed) 
```
...
Don't exclude these when checking overlap 
...
```{r}
# Are there marine only phyla?
marine_invert_irmng |> 
  filter(isTerrestrial == FALSE & isFreshwater == FALSE) |> 
  filter(class == "" & order == "" & family == "" & genus == "") -> phyla_only_data

#Yes, we can exclude these
lower_afd |> filter(PHYLUM %in% phyla_only_data$phylum) #81 species
lower_afd |> filter(! PHYLUM %in% phyla_only_data$phylum) -> afd_nomarine_phyla

# Are there terrestrial/FW Cnidaria and Poriferas? Apparently so
marine_invert_irmng |> 
...
marine_invert_irmng |> 
  filter(phylum == "Cnidaria" & isTerrestrial == TRUE) 
...
marine_invert_irmng |> 
  filter(isTerrestrial == FALSE & isFreshwater == FALSE) |> 
  filter(order == "" & family == "" & genus == "") -> class_only_data

#
afd_nomarine_phyla |> filter(CLASS %in% class_only_data$class) |> 
  select(PHYLUM:SUB_SPECIES, ends_with("NAME")) |> 
