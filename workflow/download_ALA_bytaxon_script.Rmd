...
I think the major difference between the NESP bugs and current galah workflow is that NESP bugs workflow supplies a species list from AFD `afd_taxonomy$PHYLUM` to query ALA, whereas `galah` filters by ALL INVERTEBRATES first and then downloads the occurrences. We recommend processing the download and cross match with AFD after the download. Below I have provided an option to do both! 

Some of the latter parts of NESP bugs workflow involves collapsing down the lists of download (organised by phylum).  I assumed this was a product for parallel processing using `future` so I didn't streamlined these sections.  
...
Replacing `identificationIncorrect` with whatever assertion name you’d like to use. 
...
Since there are some inverts that are in Chordata, so IA may not want to exclude all Chordates as above. The below method accepts a user supplied taxa list and searched in the ALA to obtain occurrence data. This method would also work with taxon counts above. 

The only bug with `{galah}` at the moment for this part of the workflow is looking up [taxa that have homonyms](https://github.com/AtlasOfLivingAustralia/galah-R/issues/168) with Acantocephala the Phylum.Currently there is a workaround shown below
...
I've set this is a background RStudio job for the download but for some reason it is not work. `Sys.sleep(600)` is for taking a 10 minute break so to not overload the API
...
The workflow is a bit different with parquets, this option is way faster than reading each RDS/CSV together of the mega download and doesn't use a lot of  memory. 

The idea is once you've joined them, you subset the parts you want later on. 
```{r}
...
# Saving a more particular subset of the data
IA_data %>% 
  select(recordID, scientificName, starts_with("decimal"), year) %>% 
  collect() %>% 
  filter(year >1940) %>% 
  write_parquet("output/IA_1940onwards")

# Read the subset in R 
IA_1940onwards <- read_parquet("output/IA_1940onwards")
```
...
Getting all the assertions for all records may not be so feasible. Perhaps limiting to a set amount of years, first deciding which assertions are important then running the query. 

```{r}

pacman::p_load(data.table, galah, dplyr, CoordinateCleaner, sf, ozmaps, job, arrow, purrr, data.table, ggplot2)
...
#### Read in parquets for assertions

Work in progress

```{r}
# Read in some example subset
IA_1940onwards <- open_dataset("output/IA_1940onwards")

path <- "data/galah/assertions/"
assertions_parq <- list.files(path)

# An example of joining back to data
assert_acantocephala <- open_dataset(paste0(path, assertions_parq[1]))

IA_1940onwards %>% 
  left_join(assert_acantocephala, by = "recordID") 
...
Removing NA in coordinates, similar to data cleaning steps that are in get_ala_taxondata.R
...
Update Jan 2023 CoordinateCleaner is currently broken! 
