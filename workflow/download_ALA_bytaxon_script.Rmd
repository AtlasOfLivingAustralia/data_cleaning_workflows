...
pacman::p_load(data.table, galah, dplyr, CoordinateCleaner, sf, ozmaps)
```
...
I realised there are some inverts that are in Chordata, so we definitely don't want to exclude all Chordates. This method would also work with taxon counts above
...
y_ls <- split(y, f = ceiling(seq_along(select$name)/20))

# Add id in 2nd and 3rd list so output can all be joined
...
get_occ <- function(y){
  ...
  x$select <- y
   ...
   x |> 
    atlas_occurrences() |> 
   arrow::write_parquet(sink = paste0("data/galah/IA_", Sys.Date()))
...
```{r}
test <- arrow::read_parquet("data/galah/IA_2023-01-03")
