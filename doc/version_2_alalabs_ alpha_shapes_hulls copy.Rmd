...
The ability to visualise and predict where a species reside is important in conservation ecology. In this ALA Labs post, we'll explain the difference between two commonly used methods for graphically representing species distributions and show you how to create them in R! 
...
By the end of this post, you'll be able to make a map like this! These spatial visualisations are a great alternative to plotting point based species occurrence records. We will start with **convex hulls** - the simplest of the shapes and we will end with **alpha hulls** (in our opinion the most visually appealing)...
...
### Mapping Alpha shapes of ALA occurences
...
To illustrate this concept, here are three alpha hulls with increasing values for alpha:
...
You'll notice with this map that there are **two separate hulls** this is a benefit of an alpha hull over a convex hull: the mathematical constraints of a convex hull mean all the points must be contained within a single shape-this can lead to an over-estimation of range. If you scroll all the way back up to the beginning of this post to our first map you'll see these multiple hulls quite clearly! 
The flip side of that is an alpha hull could be under-estimating range..pick your poison! 
...
Now we know how to make nice alpha hulls using citizen science data. 
This is a much clearer way plotting range changes or species distributions than simply plotting occurrence records on a map.There are more sophisticated tools for understanding the distribution of species: like species distribution models. However, sometimes you need more data then you have access to in order to run these models and this is when alpha hulls could be a good alternative.