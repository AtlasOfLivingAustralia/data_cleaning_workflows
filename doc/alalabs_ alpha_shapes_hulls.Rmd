---
title: "Creating spatial polygons in `galah`"
...
In conservation ecology, convex hulls and alpha hulls are used to [quantify IUCN metrics such as, Area of Occupancy and Extent of Occurrence](https://www.ala.org.au/spatial-portal-help/aoo/) to describe the distribution of a particular species. Typically, convex and alpha hulls are used when dealing with **data deficient species** as opposed to Species Distribution Models which requires a large number of observations.

Here we'll walk you through how to create your own convex and alpha hulls using ALA data and showcase some **recent research** that applies these to better understand biodiversity conservation. These spatial visualizations generated are a great alternative to plotting point based species occurrence records!
...
### First, a demonstration with `iris` dataset

A convex hull is defined as the smallest polygon that encloses all the points in the dataset. Put simply, its a way to draw around all the points with as few lines as possible! 

Here's a convex hull surround the data points in the `iris` dataset. 
...
# create the outline of the hull but joining the hull vector together 
...
A convex hull is actually a simplified form of an alpha shape. Unlike the convex hull, alpha shapes have a alpha parameter that controls the level of detail, or how tightly the boundary fits around the point set.

Here are some alpha shapes with increasing values for alpha with the `iris` dataset.
...
<center>![A Powdered Flatwing perched on a plant](../static/damsel/Austroargiolestes_calcaris_1.jpeg){width="7.9cm"}</center>

We'll use `galah` to access records from the Atlas of Living Australia. Note that you will need to first set up your own [ALA configuration locally](http://galah.ala.org.au/articles/download_data.html?q=config#configuring-galah) using `galah_config` before this step.
...
It's important to make sure you've removed **all duplicates and missing values** from the data in order to plot your polygons.
...
# 3. Getting a polygon of Australia and set it to the specific coordinate system
aus <- st_transform(ozmaps::ozmap_country, 4326)

# 4. Plot the occurences and convex hull
ggplot() + 
...
Next, we can plot an alpha shape for our Powdered flatwing onto a map! We've set **alpha = 2** as this is commonly used in the research papers we've seen but choose an alpha value that best suits your needs.

We will need to use this customised function named `extract_polygons` for extracting the polygon outline from an `ashape` object as a plottable data frame. 
...
# 2. Extract the `ashape` as a polygon dataset
...
### Side-by-side of alpha shape and alpha hull maps

The biggest difference you might notice here is the alpha hull doesn't cross the ocean. This can be useful if you're trying to use this shape to calculate the area the species resides in. *Austroargiolestes calcaris* is terrestrial!
...
Often citizen science data is excluded from scientific research due to it's low data quality. You'll notice that when we exclude citizen science data for this species our number of records drop significantly. However, just because we've lost more than half our records doesn't mean it'll change the alpha hull much! This is the beauty of alpha hulls!
...
We are going to repeat the above steps for generating an alpha hull excluding citizen science data. Remember you still need to clean this data set.
...
Let's plot our polygons together!
...
Researchers such as [Takach et al., (2020)](https://onlinelibrary.wiley.com/doi/10.1111/ddi.13145) used alpha hulls to understand how the Australian tropical savanna mammal distributions have changed over time. For example, distributions can shrink or expand due to habitat loss or in response to changing climate. Takach et al., (2020) found that the [Black-Footed Tree Rat *Mesembriomys gouldii*](https://biocache.ala.org.au/occurrences/c6e1ff2d-ac80-4cc1-806c-6107e8e24617) experienced a range contraction.

<center>![A Black-Footed Tree Rat perched on a branch](../static/polygons/Mesembriomys_gouldii_1.jpeg){width="7.9cm"}</center>

The research paper didn't provide visualiations of the alpha hulls, so here we've plotted what the range reduction looks like using all records available vs. only records after the year 2000 for the Black-Footed Tree Rat using the workflow we've outlined above.
...
ggplot() + 
  ...
  labs(title = rat_title, x = "Longtitude (DD)", y = "Latitude (DD)") + 
  theme_bw() -> combinedtree_rat_ahull_p
...
We'd also recommend you go check out [Marsh et al., (2022)]() research using alpha hulls to calculate the impact the 2020 mega fires had on invertebrates in southern Australia. You'll find some nice maps of *Austroargiolestes calcaris* in their paper! Data for invertebrates are inherently sparse so alpha hulls can be helpful when you only have a handful of records to work with.
