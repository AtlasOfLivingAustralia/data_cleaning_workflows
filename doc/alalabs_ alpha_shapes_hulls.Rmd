...
Put simply a convex hull is a way to draw around all the points of a species on a map with as few lines as possible. It's defined as the smallest polygon that encloses all the points in the data set. To plot a convex hull on a map:

1. We will start by using `chull()` to compute a convex hull from our cleaned Powdered Flatwing data.
2. We'll then join the hull vector together to create an outline of the hull which can be plotted on a map. 
3. To make a map, we'll use the `ozmaps` package and set it to a specific coordinate system for Australia. 
...
Lets make some alpha shapes for the Powdered Flatwing with increasing values of alpha. In order to do this we first need to use the `purrr` R package and a customised function named `extract_polygons` written by [Cecina Babich Morrow](https://babichmorrowc.github.io/) for extracting the polygon outline from an `ashape` object so we can plot it with `ggplot2`.

```{r extract_polygons, class.source = "fold-hide"}
...
Recently, [Dr. Marsh and colleages](https://onlinelibrary.wiley.com/doi/full/10.1111/geb.13550) used alpha hulls to calculate the impact the 2020 mega bush fires on invertebrates in southern Australia. This research was the impetus for this post because invertebrate data is inherently sparse so alpha hulls can be helpful when you only have a handful of records to work with. 

[Dr. Takach and their team](https://onlinelibrary.wiley.com/doi/10.1111/ddi.13145) used alpha hulls to understand how the Australian tropical savanna mammal distributions have changed over time. For example, distributions can shrink or expand due to habitat loss or in response to changing climate. Dr. Takach found that the [Black-Footed Tree Rat *Mesembriomys gouldii*](https://biocache.ala.org.au/occurrences/c6e1ff2d-ac80-4cc1-806c-6107e8e24617) experienced a range contraction. We've plotted this range contraction using all the things we've just learnt about alpha hulls! We've used `galah` to download all records available for the Black-Footed Tree Rat and only records recorded after 2000. 
