...
inverts <- read_csv(here("R", "alalabs", "ignore", "Curated_Plant_and_Invertebrate_Data_for_Bushfire_Modelling", "invertebrate.data.csv"))
...
fire <- st_read(here("R", "alalabs", "ignore", "NIAFED_v20200623", "NIAFED_20190701_20200622_v20200623.shp")) %>% 
  st_transform(crs = 4326) %>% 
  st_zm() %>%   #Remove Z or M values
...
saveRDS(inverts_nest_concave, here("R", "alalabs","output", "processed_data", "inverts_nest_concave"))

inverts_nest_concave <- readRDS(here("R", "alalabs", "output", "processed_data", "inverts_nest_concave"))
```
...
#bess_nest_rng_overlap <- readRDS(here("R", "alalabs", "ignore", "bees_overlap", "bess_nest_rng_overlap"))
...
```{r}
# Read in data
vplants <- read_csv(here("R", "alalabs","ignore","Curated_Plant_and_Invertebrate_Data_for_Bushfire_Modelling", "vascularplant.data.csv"))
...
## all plant overlap
#vplant_overlap <- readRDS(here("R", "alalabs", "ignore", "vplant_overlap", "vplant_overlap")) %>% unnest(cols = c(overlap_area, percent_overlap))

##filter to Fabaceae 

pea_subset <- vplant_overlap %>%
  filter(family == "Fabaceae") 
...
Daviesia<-pea_nest_rng_overlap %>%
  filter(genus == "Daviesia")


## Prepare for plotting
pea_plotready <- Daviesia %>% 
  select(scientific_name, overlap_area, percent_overlap, points_sf, concave_sf, overlap_sf) %>% 
  unnest() %>% 
  rename(points = geometry, 
         concave = polygons, 
...
# Preview in Finder
ggsave("Dav3_pea_main.png", device = "png", 
       path = here("R", "alalabs",  "output", "fig"),
       bg = "white")
```
...
```{r}
main_bbox2 <- main_map2 + 
   geom_rect(aes(xmin = 145.5, xmax = 150.3,
             ymin = -35, ymax = -37.95),
             colour = "black",
             fill = NA, linewidth = 0.2) + 
  geom_rect(aes(xmin = 151.5, xmax = 153,
             ymin = -28.3, ymax = -31.5),
             colour = "black",
             fill = NA, linewidth = 0.2)
...
```{r}
ggdraw(main_bbox2) +
  draw_plot(inset2, x = 0.5, y = 0.2, 
            width =0.4, height = 0.2) +
  draw_plot(inset3, x = 0.44, y = 0.51, 
            width =0.4, height = 0.2)
  


ggsave("Dav3_pea_main.inset2.png", device = "png", 
       path = here("R", "alalabs",  "output", "fig"),
       bg = "white")
```
