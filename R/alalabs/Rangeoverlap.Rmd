...
pacman::p_load(tidyverse, here, sf, ozmaps, concaveman, job)
```
...
saveRDS(inverts_nest_2, here("output", "processed_data", "inverts_nest_2"))
```
...
```{r}
# Unnest the data
all_species_concave <- inverts_nest_2 %>% 
...
base_map <- ggplot() + 
  ...
  geom_sf(data = fire, fill = "#15133C", colour = "#15133C")


base_map + 
  geom_sf(data = all_species_concave, aes(geometry = polygons, colour = scientific_name), size = 1.5, alpha = 0.2, fill = NA) + 
...
```{r}
phasmids_nest <- inverts_nest_2 %>% 
  filter(family == "phasmatidae") 
```

### Compute fire overlap and calculate the area of overlap

This step takes a long time if you want to apply it across all species in the dataset. Recommend subsetting a family for this walk through or running as a background job 

```{r, warning=FALSE}
sf_use_s2(FALSE)

phasmin_nest_rng_overlap <- phasmids_nest %>% 
  mutate(overlap_sf = map(concave_sf, 
                       ~ st_intersection(fire, .x)
                       ),
         overlap_area = map(overlap,
                              ~ st_area(.x)
                              )
  ) %>% 
  unnest(cols = overlap_area)

phasmin_nest_rng_overlap %>% pluck("overlap",1)
```

## Rank species by their area overlap with fire
(Don't think we need this section now that we have subsetted the data down)
...
```{r, warnings = FALSE}
### Unnest
phasmid_plotready <- phasmin_nest_rng_overlap %>% 
  select(scientific_name, overlap_area, points_sf, concave_sf, overlap) %>% 
  unnest() %>% 
  rename(points = geometry, 
         concave = polygons, 
         overlap = geometry1) %>% 
...
phasmid_plotready %>% 
  filter(scientific_name %in% c("Megacrania batesii",
                                "Anchiale spinicollis",
                                "Ctenomorpha marginipennis")) -> test
```
...
base_map_simplify + 
  geom_sf(data = test, aes(geometry = overlap), colour = "#FF925C", fill = "#FF925C") + 
  geom_sf(data = test, aes(geometry = concave, colour = scientific_name, fill = scientific_name), size = 1.5, alpha = 0.001) + 
  geom_sf(data = test, aes(geometry = points, colour = scientific_name), size = 1) + 
  scale_colour_manual(values = c("#023E50", "#7B8A6A", "#3C908E")) + 
  scale_fill_manual(values = c("#023E50", "#7B8A6A", "#3C908E")) + 
  coord_sf(xlim=c(130, 155),
          ylim=c(-10,-45)) +
  guides(colour = guide_legend(override.aes = list(alpha = 0.02))) + 
  theme_minimal() + 
  theme(legend.position = "bottom") 


base_map_simplify + 
  geom_sf(data = phasmid_plotready, aes(geometry = overlap), colour = "#FF925C", fill = "#FF925C") + 
  geom_sf(data = phasmid_plotready, aes(geometry = concave, colour = scientific_name, fill = scientific_name), size = 1.5, alpha = 0.001) + 
  geom_sf(data = phasmid_plotready, aes(geometry = points, colour = scientific_name), size = 1) + 
  # scale_colour_manual(values = c("#023E50", "#7B8A6A", "#3C908E")) + 
  # scale_fill_manual(values = c("#023E50", "#7B8A6A", "#3C908E")) + 
  # coord_sf(xlim=c(130, 155),
  #         ylim=c(-10,-45)) +
