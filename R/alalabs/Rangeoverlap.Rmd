...
pacman::p_load(tidyverse, here, rmapshaper, sf, ozmaps, concaveman, job, cowplot)
```
...
```
```{r loaddatalocal, echo = FALSE, include = FALSE}
...
Click on the "Download Data" button on the first filename ("NIAFED_v20200623.zip") to download the .zip file. 

![]( `r here("images", "Firelayer.png")` ) 

Similar to above, move the downloaded .zip to an appropriate folder and then double click on it to unzip its contents. Once that is done, we can read it into R.

We will also  set the projection to EPSG:4326, remove elevation values as we are only interested in the extent of fire and simplify the polygon.
...
```{r firelayerlocal, echo = FALSE, include = FALSE}
...
Have a look at the first element of each of these new variables look like using `purrr::pluck`
...
Now we can rank all bee species by the percentage range overlap with fire extent, then take the top 3 species with the highest overlap
...
### A glimpse of spatial distributions of the Vascular plant dataset

(Consider moving this put in intro)
...
# saveRDS(vplant_overlap, "output/rds/vplant_overlap")


## Subset
subset_nest_concave <- vplant_overlap %>%
...
top3 <- subset_nest_concave %>% 
  slice_max(order_by = percent_overlap,
              n = 3)  %>% 
  slice_sample(n = 3)

## Prepare for plotting
plant_plotready <- top3 %>% 
  select(scientific_name, overlap_area, percent_overlap, points_sf, concave_sf, overlap_sf) %>% 
  unnest() %>% 
  rename(points = geometry, 
         concave = polygons, 
...
### Visualise the ferns/subgroup/XX

```{r}
# Overlap map
main_map <- base_map + 
  geom_sf(data = plant_plotready, aes(geometry = overlap), colour = "#FF925C", fill = "#FF925C") + 
  geom_sf(data = plant_plotready, aes(geometry = concave, colour = scientific_name, fill = scientific_name), size = 1.5, alpha = 0.0045) + 
  geom_sf(data = plant_plotready, aes(geometry = points, colour = scientific_name), size = 1) + 
  scale_colour_manual(values = c("#023E50", "#7B8A6A", "#3C908E")) + 
  scale_fill_manual(values = c("#023E50", "#7B8A6A", "#3C908E")) + 
  guides(colour = guide_legend(override.aes = list(alpha = 1))) + 
  coord_sf(xlim = c(140, 158)) +
...
```{r}
# # Preview in Finder
# ggsave("plant_main.png", device = "png", 
