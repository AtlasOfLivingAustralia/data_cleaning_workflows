...
Just like we did for the invertebrate data, we need to compute concave hulls for our chosen species, in this case we will just do the 3 *Daviesia* species. We can then compute the range with the bush-fire layer followed plotting our outputs.
...
You'll might notice that *Daviesia buxifolia* has a range percentage overlap of 37% which is below the 50% overlap of the data. This is likely caused by a variation in the way that species range overlaps are being calculated, with hulls being coarse, and tending to over-estimate range, you can end up with an underestimation of range overlap. We'll take a look at *D. Buxifolia* when it's plotted below to see if this could be the case. 
```{r}
daviesia_overlap
```
...
Now that we have the 3 *Daviesa* hulls plotted, we will make some insets like we did for the bees, as these species ranges are all quite small, we can make multiple and then add them all onto the main map at the end. The insets provide a good idea of jsut how burnt
...
daviesia_inset_1 + daviesia_inset_2 
```

Inset 1 in dark blue is *D. buxifolia*, if you look at where the majority of the points are located compared with the hull, there is a lot of un-burnt land included in it's range. Now this range isn't necessarily wrong, it's purely an estimate, however it can help to explain disparity in findings when different methods are used.  
