...
```{r lib}
pacman::p_load(tidymodels, rpart, rpart.plot, tidyverse, data.table, skimr, janitor, rattle)
```
...
invert_irmng_extant |> 
  select(phylum:genus, starts_with("is"), -isExtinct) |> 
...
```{r}
train_data |> select(-starts_with("is")) -> rpart_dat
